        ...
        <div class="sidebar-nav navbar-collapse">
            <ul class="nav" id="side-menu">
                ...
                <li style="border: 0px;"><br>
                    <a href="Existing-Industrial-FMA">
...
if(isset($_GET['id'])){
  $id = $_GET['id'];
  $edit = mysqli_query($conn, "SELECT * FROM existing_industrial_forest_management_area WHERE id = '$id'");
  $row = mysqli_fetch_array($edit);
}elseif(isset($_GET['del'])){
    $id = $_GET['del'];
    $sql = mysqli_query($conn, "DELETE FROM existing_industrial_forest_management_area WHERE id = '$id'");
    header("location:Existing-Industrial-FMA");
}
?>
...
mysqli_query($conn,"UPDATE existing_industrial_forest_management_area set Penro='$Penro',Cenro='$Cenro',name_of_holder='$name_of_holder',Location='$Location',Area_ha='$Area_ha',Area_planted_ha='$Area_planted_ha',Status='$Status' where id='$id'");
echo "<script>alert('Success! Record Updated.'); window.location='Existing-Industrial-FMA'</script>";
            ...
            <div class="panel-heading" style="letter-spacing:2px; font-size:20px; font-weight: bold;"><img src="img/Info Squared_96px.png"  height="40px" width="40px">EDIT RECORD</div>
                  <div class="panel-body">
                       <form method="POST" class="form-horizontal" autocomplete="off">
                              ...
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Name of Holder</small></label>
                                       <div class="col-sm-10">          
                                          <input name="name_of_holder" type="text" class="form-control" required value="<?= $row['name_of_holder']; ?>">
                                       </div>
                              </div>
