...
<?php
	...
	header("Content-Disposition: attachment; filename=Confiscated Conveyances By Year.xls");  
	...
	$output .="
		<table>
			<h2 style='color:#008B8B;'>ENR Profile : Confiscated Conveyances, By Year, Region 10</h2>
			<thead> <br>
	...
	$query = $conn->query("SELECT * FROM `confiscated_conveyances` where OperatingUnit='Non-PA' OR OperatingUnit='Mt. Timpoong Hibok-Hibok'  ORDER BY year ASC") or die(mysqli_errno());
	while($fetch = $query->fetch_array()){
	...
	$query = $conn->query("SELECT * FROM `confiscated_conveyances` where OperatingUnit='CENRO Ozamiz City' OR OperatingUnit='CENRO Oroquieta City' OR OperatingUnit='Mt. Malindang RNP' OR OperatingUnit='Mt. Baliangao PLS' ORDER BY year ASC") or die(mysqli_errno());
	while($fetch = $query->fetch_array()){
	...
	$query = $conn->query("SELECT * FROM `confiscated_conveyances` where OperatingUnit='CENRO Initao' OR OperatingUnit='CENRO Gingoog City' OR OperatingUnit='Mt. Balatukan' OR OperatingUnit='Mimbilisan' OR OperatingUnit='Initao Libertad' ORDER BY year ASC") or die(mysqli_errno());
	while($fetch = $query->fetch_array()){
