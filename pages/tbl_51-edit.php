        ...
        <div class="sidebar-nav navbar-collapse">
            <ul class="nav" id="side-menu">
                ...
                <li style="border: 0px;"><br>
                    <a href="Seedling-Planted-by-Commodity">
...
if(isset($_GET['id'])){
  $id = $_GET['id'];
  $edit = mysqli_query($conn, "SELECT * FROM ngp_seedlings_planted_by_commodity WHERE id = '$id'");
  $row = mysqli_fetch_array($edit);
}elseif(isset($_GET['del'])){
    $id = $_GET['del'];
    $sql = mysqli_query($conn, "DELETE FROM ngp_seedlings_planted_by_commodity WHERE id = '$id'");
    header("location:Seedling-Planted-by-Commodity");
}
?>
...
mysqli_query($conn,"UPDATE ngp_seedlings_planted_by_commodity set commodity='$commodity',number_of_seedlings_planted='$number_of_seedlings_planted',year='$year' where id='$id'");
echo "<script>alert('Success! Record Updated.'); window.location='Seedling-Planted-by-Commodity'</script>";
                ...
                <div class="panel-body">
                       <form method="POST" class="form-horizontal" autocomplete="off">
                              ...
                              <div class="form-group container-fluid">
                                    <label for="usr"><small>Commodity</small></label>
                                    <input name="commodity" type="text" class="form-control" required value="<?= $row['commodity']; ?>">
                                
                                    <label for="usr"><small>No. of Commodity/Seedlings Planted</small></label>
                                    <input name="number_of_seedlings_planted" type="text" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" class="form-control" required value="<?= $row['number_of_seedlings_planted']; ?>">

                                    <label for="usr"><small>Year</small></label>
