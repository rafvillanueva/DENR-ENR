...
if(isset($_GET['id'])){
  $id = $_GET['id'];
  $edit = mysqli_query($conn, "SELECT * FROM Tenurial_Instruments WHERE id = '$id'");
  $row = mysqli_fetch_array($edit);
}elseif(isset($_GET['del'])){
    $id = $_GET['del'];
    $sql = mysqli_query($conn, "DELETE FROM Tenurial_Instruments WHERE id = '$id'");
    header("location:Tenurial-Instruments");
} 
?>
...
mysqli_query($conn,"UPDATE Tenurial_Instruments set Tenurial_Instruments='$Tenurial_Instruments',No_of_Holders='$No_of_Holders',Area_ha='$Area_ha' where id='$id'");
echo "<script>alert('Success! Record Updated.'); window.location='Tenurial-Instruments'</script>";
            ...
            <div class="panel-heading" style="letter-spacing:2px; font-size:20px; font-weight: bold;"><img src="img/Info Squared_96px.png"  height="40px" width="40px">EDIT RECORD</div>
                  <div class="panel-body">
                       <form method="POST" class="form-horizontal" autocomplete="off">
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Tenurial Instruments</small></label>
                                       <div class="col-sm-10">          
                                          <input name="Tenurial_Instruments" type="text" class="form-control" required value="<?= $row['Tenurial_Instruments']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>No. of Holders</small></label>
                                       <div class="col-sm-10">          
                                          <input name="No_of_Holders" type="text" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" class="form-control" required value="<?= $row['No_of_Holders']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
