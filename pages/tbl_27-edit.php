...
if(isset($_GET['id'])){
  $id = $_GET['id'];
  $edit = mysqli_query($conn, "SELECT * FROM existing_flgma WHERE id = '$id'");
  $row = mysqli_fetch_array($edit);
}elseif(isset($_GET['del'])){
    $id = $_GET['del'];
    $sql = mysqli_query($conn, "DELETE FROM existing_flgma WHERE id = '$id'");
    header("location:Existing_FLGMA");
}
?>
...
mysqli_query($conn,"UPDATE existing_flgma set Penro='$Penro',Cenro='$Cenro',Municipality_City='$Municipality_City',Barangay='$Barangay',Name_of_instrument='$Name_of_instrument',Area_ha='$Area_ha',Status='$Status' where id='$id'");
echo "<script>alert('Success! Record Updated.'); window.location='Existing_FLGMA'</script>";
            ...
            <div class="panel-heading" style="letter-spacing:2px; font-size:20px; font-weight: bold;"><img src="img/Info Squared_96px.png"  height="40px" width="40px">EDIT RECORD</div>
                  <div class="panel-body">
                       <form method="POST" class="form-horizontal" autocomplete="off">
                              ...
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Municipality/City</small></label>
                                       <div class="col-sm-10">          
                                          <input name="Municipality_City" type="text" class="form-control" required value="<?= $row['Municipality_City']; ?>">
                                       </div>
                              </div>

                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Barangay</small></label>
                                       <div class="col-sm-10">          
                                          <input name="Barangay" type="text" class="form-control" required value="<?= $row['Barangay']; ?>">
                                       </div>
                              </div>

                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Name of Instrument Holder/Leasee</small></label>
                                       <div class="col-sm-10">          
                                          <input name="Name_of_instrument" type="text" class="form-control" required value="<?= $row['Name_of_instrument']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
