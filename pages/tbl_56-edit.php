        ...
        <div class="sidebar-nav navbar-collapse">
            <ul class="nav" id="side-menu">
                ...
                <li style="border: 0px;"><br>
                    <a href="Number-of-Incidents-on-Apprehended">
...
if(isset($_GET['id'])){
  $id = $_GET['id'];
  $edit = mysqli_query($conn, "SELECT * FROM number_of_incidents_on_apprehended WHERE id = '$id'");
  $row = mysqli_fetch_array($edit);
}elseif(isset($_GET['del'])){
    $id = $_GET['del'];
    $sql = mysqli_query($conn, "DELETE FROM number_of_incidents_on_apprehended WHERE id = '$id'");
    header("location:Number-of-Incidents-on-Apprehended");
} 
?>
...
mysqli_query($conn,"UPDATE number_of_incidents_on_apprehended set OperatingUnit='$OperatingUnit',No_of_Incidents='$No_of_Incidents',Year='$Year' where id='$id'");
echo "<script>alert('Success! Record Updated.'); window.location='Number-of-Incidents-on-Apprehended'</script>";
            ...
            <div class="panel-heading" style="letter-spacing:2px; font-size:20px; font-weight: bold;"><img src="img/Info Squared_96px.png"  height="40px" width="40px">EDIT RECORD</div>
                <div class="panel-body">
                       <form method="POST" class="form-horizontal" autocomplete="off">
                              ...
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>No. of Incidents</small></label>
                                       <div class="col-sm-10">          
                                          <input name="No_of_Incidents" type="text" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" class="form-control" required value="<?= $row['No_of_Incidents']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
