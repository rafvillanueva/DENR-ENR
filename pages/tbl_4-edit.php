        ...
        <div class="sidebar-nav navbar-collapse">
            <ul class="nav" id="side-menu">
                ...
                <li style="border: 0px;"><br>
                    <a href="Apprehended-Implements-Equipment">
...
if(isset($_GET['id'])){
  $id = $_GET['id'];
  $edit = mysqli_query($conn, "SELECT * FROM  apprehended_implements_equipment WHERE id = '$id'");
  $row = mysqli_fetch_array($edit);
}elseif(isset($_GET['del'])){
    $id = $_GET['del'];
    $sql = mysqli_query($conn, "DELETE FROM   apprehended_implements_equipment WHERE id = '$id'");
    header("location:Apprehended-Implements-Equipment");
}
?>
...
mysqli_query($conn,"UPDATE  apprehended_implements_equipment set NoUnits='$NoUnits',EstValue='$EstValue',Year='$Year',OperatingUnit='$OperatingUnit' where id='$id'");
echo "<script>alert('Success! Record Updated.'); window.location='Apprehended-Implements-Equipment'</script>";
            ...
            <div class="panel-heading" style="letter-spacing:2px; font-size:20px; font-weight: bold;"><img src="img/Info Squared_96px.png"  height="40px" width="40px">EDIT RECORD</div>
                  <div class="panel-body">
                       <form method="POST" class="form-horizontal" autocomplete="off">
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>No. of Units</small></label>
                                       <div class="col-sm-10">          
                                          <input name="NoUnits" type="text" class="form-control" required value="<?= $row['NoUnits']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
