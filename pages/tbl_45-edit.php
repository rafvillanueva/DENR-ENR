...
if(isset($_GET['id'])){
  $id = $_GET['id'];
  $edit = mysqli_query($conn, "SELECT * FROM major_river_basins WHERE id = '$id'");
  $row = mysqli_fetch_array($edit);
}elseif(isset($_GET['del'])){
    $id = $_GET['del'];
    $sql = mysqli_query($conn, "DELETE FROM major_river_basins WHERE id = '$id'");
    header("location:Major-River-Basins");
}
?>
...
mysqli_query($conn,"UPDATE major_river_basins set River_basins='$River_basins',area_ha='$area_ha',provinces_covered='$provinces_covered',city_municipality_covered='$city_municipality_covered',river_tributary='$river_tributary',outlet='$outlet' where id='$id'");
echo "<script>alert('Success! Record Updated.'); window.location='Major-River-Basins'</script>";
            ...
            <div class="panel-heading" style="letter-spacing:2px; font-size:20px; font-weight: bold;"><img src="img/Info Squared_96px.png"  height="40px" width="40px">EDIT RECORD</div>
                <div class="panel-body">
                       <form method="POST" class="form-horizontal" autocomplete="off">
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>River Basins</small></label>
                                       <div class="col-sm-10">          
                                          <input name="River_basins" type="text" class="form-control" required value="<?= $row['River_basins']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Area (ha)</small></label>
                                       <div class="col-sm-10">          
                                          <input name="area_ha" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" type="text" class="form-control" required value="<?= number_format($row['area_ha']); ?>">
                                       </div>
                              </div>
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Provinces Covered</small></label>
                                       <div class="col-sm-10">          
                                          <input name="provinces_covered" type="text" class="form-control" required value="<?= $row['provinces_covered']; ?>">
                                       </div>
                              </div>

                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>City/Municipality Covered</small></label>
                                       <div class="col-sm-10">          
                                          <input name="city_municipality_covered" type="text" class="form-control" required value="<?= $row['city_municipality_covered']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>River Tributary</small></label>
                                       <div class="col-sm-10">          
                                          <input name="river_tributary" type="text" class="form-control" required value="<?= $row['river_tributary']; ?>">
                                       </div>
                              </div>

                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Outlet</small></label>
                                       <div class="col-sm-10">          
                                          <input name="outlet" type="text" class="form-control" required value="<?= $row['outlet']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">        
