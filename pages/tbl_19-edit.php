...
if(isset($_GET['id'])){
  $id = $_GET['id'];
  $edit = mysqli_query($conn, "SELECT * FROM cbep_persons_employed WHERE id = '$id'");
  $row = mysqli_fetch_array($edit);
}elseif(isset($_GET['del'])){
    $id = $_GET['del'];
    $sql = mysqli_query($conn, "DELETE FROM cbep_persons_employed WHERE id = '$id'");
    header("location:CBEP-persons-employed");
}
?>
...
mysqli_query($conn,"UPDATE cbep_persons_employed set penro_cenro='$penro_cenro',numbers='$numbers',year='$year' where id='$id'");
echo "<script>alert('Success! Record Updated.'); window.location='CBEP-persons-employed'</script>";
            ...
            <div class="panel-heading" style="letter-spacing:2px; font-size:20px; font-weight: bold;"><img src="img/Info Squared_96px.png"  height="40px" width="40px">EDIT RECORD</div>
                  <div class="panel-body">
                       <form method="POST" class="form-horizontal" autocomplete="off">
                              ...
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Number of persons employed</small> </label>
                                       <div class="col-sm-10">          
                                          <input name="numbers" type="text" class="form-control" required value="<?= $row['numbers']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
