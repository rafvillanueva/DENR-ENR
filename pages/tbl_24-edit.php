...
if(isset($_GET['id'])){
  $id = $_GET['id'];
  $edit = mysqli_query($conn, "SELECT * FROM critical_watersheds WHERE id = '$id'");
  $row = mysqli_fetch_array($edit);
}elseif(isset($_GET['del'])){
    $id = $_GET['del'];
    $sql = mysqli_query($conn, "DELETE FROM critical_watersheds WHERE id = '$id'");
    header("location:Critical-Watersheds");
}
?>
...
mysqli_query($conn,"UPDATE critical_watersheds set Province='$Province',Name_watershed='$Name_watershed',Area_ha='$Area_ha' where id='$id'");
echo "<script>alert('Success! Record Updated.'); window.location='Critical-Watersheds'</script>";
                       ...
                       <form method="POST" class="form-horizontal" autocomplete="off">
                              ...
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Name of Watershed</small></label>
                                       <div class="col-sm-10">          
                                          <input name="Name_watershed" type="text" class="form-control" required value="<?= $row['Name_watershed']; ?>">
                                       </div>
                              </div>
