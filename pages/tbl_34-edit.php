...
if(isset($_GET['id'])){
  $id = $_GET['id'];
  $edit = mysqli_query($conn, "SELECT * FROM forest_cover WHERE id = '$id'");
  $row = mysqli_fetch_array($edit);
}elseif(isset($_GET['del'])){
    $id = $_GET['del'];
    $sql = mysqli_query($conn, "DELETE FROM forest_cover WHERE id = '$id'");
    header("location:Forest-Cover");
}
?>
...
mysqli_query($conn,"UPDATE forest_cover set Province='$Province',Close='$Close',Open='$Open',Mangrove='$Mangrove',Total='$Total' where id='$id'");
echo "<script>alert('Success! Record Updated.'); window.location='Forest-Cover'</script>";
            ...
            <div class="panel-heading" style="letter-spacing:2px; font-size:20px; font-weight: bold;"><img src="img/Info Squared_96px.png"  height="40px" width="40px">EDIT RECORD</div>
                  <div class="panel-body">
                       <form method="POST" class="form-horizontal" autocomplete="off">
                              ...
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Closed</small></label>
                                       <div class="col-sm-10">          
                                          <input name="Close" type="text" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" class="form-control" required value="<?= $row['Close']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Open</small></label>
                                       <div class="col-sm-10">          
                                          <input name="Open" type="text" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" class="form-control" required value="<?= $row['Open']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Mangrove</small></label>
                                       <div class="col-sm-10">          
                                          <input name="Mangrove" type="text" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" class="form-control" required value="<?= $row['Mangrove']; ?>">
                                       </div>
                              </div>
                                       <div class="form-group">
                                
                                       <div class="col-sm-10">          
                                          <input name="Total" type="hidden" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" class="form-control" required value="<?= $row['Total']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">        
