...
if(isset($_GET['id'])){
  $id = $_GET['id'];
  $edit = mysqli_query($conn, "SELECT * FROM existing_slup WHERE id = '$id'");
  $row = mysqli_fetch_array($edit);
}elseif(isset($_GET['del'])){
    $id = $_GET['del'];
    $sql = mysqli_query($conn, "DELETE FROM existing_slup WHERE id = '$id'");
    header("location:Existing-SLUP");
}
?>
...
mysqli_query($conn,"UPDATE existing_slup set Penro='$Penro',Cenro='$Cenro',Location='$Location',Name_of_Permittee='$Name_of_Permittee',Type_of_slup='$Type_of_slup',Area_ha='$Area_ha',Status='$Status' where id='$id'");
echo "<script>alert('Success! Record Updated.'); window.location='Existing-SLUP'</script>";
            ...
            <div class="panel-heading" style="letter-spacing:2px; font-size:20px; font-weight: bold;"><img src="img/Info Squared_96px.png"  height="40px" width="40px">EDIT RECORD</div>
                  <div class="panel-body">
                       <form method="POST" class="form-horizontal" autocomplete="off">
                              ...
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Name of Permittee</small></label>
                                       <div class="col-sm-10">          
                                          <input name="Name_of_Permittee" type="text" class="form-control" required value="<?= $row['Name_of_Permittee']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Type of slup</small></label>
                                       <div class="col-sm-10">          
                                          <input name="Type_of_slup" type="text"  class="form-control" required value="<?= $row['Type_of_slup']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
