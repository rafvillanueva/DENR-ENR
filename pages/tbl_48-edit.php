...
if(isset($_GET['id'])){
  $id = $_GET['id'];
  $edit = mysqli_query($conn, "SELECT * FROM ngp_number_of_polygons WHERE id = '$id'");
  $row = mysqli_fetch_array($edit);
}elseif(isset($_GET['del'])){
    $id = $_GET['del'];
    $sql = mysqli_query($conn, "DELETE FROM ngp_number_of_polygons WHERE id = '$id'");
    header("location:Number-of-Polygons");
}
?>
...
mysqli_query($conn,"UPDATE ngp_number_of_polygons set province='$province',no_of_polygons='$no_of_polygons',no_of_sites='$no_of_sites',no_of_beneficiaries='$no_of_beneficiaries' where id='$id'");
echo "<script>alert('Success! Record Updated.'); window.location='Number-of-Polygons'</script>";
            ...
            <div class="panel-heading" style="letter-spacing:2px; font-size:20px; font-weight: bold;"><img src="img/Info Squared_96px.png"  height="40px" width="40px">EDIT RECORD</div>
                <div class="panel-body">
                       <form method="POST" class="form-horizontal" autocomplete="off">
                              ...
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>No. of Polygons </small></label>
                                       <div class="col-sm-10">          
                                          <input name="no_of_polygons" type="text" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" class="form-control" required value="<?= $row['no_of_polygons']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>No. of Sites</small></label>
                                       <div class="col-sm-10">          
                                          <input name="no_of_sites" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" type="text" class="form-control" required value="<?= $row['no_of_sites']; ?>">
                                       </div>
                              </div>


                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>No. of Beneficiaries</small></label>
                                       <div class="col-sm-10">          
                                          <input name="no_of_beneficiaries" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" type="text" class="form-control" required value="<?= $row['no_of_beneficiaries']; ?>">
                                       </div>
                              </div>
