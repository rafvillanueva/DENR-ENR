...
if(isset($_GET['id'])){
  $id = $_GET['id'];
  $edit = mysqli_query($conn, "SELECT * FROM ngp_seedling_production WHERE id = '$id'");
  $row = mysqli_fetch_array($edit);
}elseif(isset($_GET['del'])){
    $id = $_GET['del'];
    $sql = mysqli_query($conn, "DELETE FROM ngp_seedling_production WHERE id = '$id'");
    header("location:Seedling-Production");
}
?>
...
mysqli_query($conn,"UPDATE ngp_seedling_production set province='$province',number_of_seedlings_production='$number_of_seedlings_production',year='$year' where id='$id'");
echo "<script>alert('Success! Record Updated.'); window.location='Seedling-Production'</script>";
            ...
            <div class="panel-heading" style="letter-spacing:2px; font-size:20px; font-weight: bold;"><img src="img/Info Squared_96px.png"  height="40px" width="40px">EDIT RECORD</div>
                <div class="panel-body">
                       <form method="POST" class="form-horizontal" autocomplete="off">
                              ...
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>No. of Seedling production</small> </label>
                                       <div class="col-sm-10">          
                                          <input name="number_of_seedlings_production" type="text" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" class="form-control" required value="<?= $row['number_of_seedlings_production']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
