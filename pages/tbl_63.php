     ...
     <div class="row1">
                      <label class="col-sm-12" style="font-weight:bold; margin-left:-15px; letter-spacing:1px; font-family:Calibri;font-size: 18px; color:#FE4A49;"> SELECT A RECORDS</label>
                            ...
                            <select class="form-control" onchange="myFunction()" id="mySelect" required style="border:1px solid gray; letter-spacing: 2px; font-size:17px;">
                              ...
                              <option selected value="1" class="title-op">Summary of Watershed Characterization, VA and IWMP, Region 10</option>
...
<!--/ end -->
      ...
      <button class="btn btn-default btn btn-md" data-toggle="modal" data-target="#tbl63-add" style="height:40px; background:#001730; color:white; opacity:0.8;"><span class="fa fa-plus"></span> Add</button>

      <button class="btn btn-default btn btn-md"  data-toggle="modal" data-target="#tbl63-total"  style="background:#FE4A49; color:white; height:40px;"><span class="fa fa-calculator"></span> Total</button>


      <a href="report/tbl63.php">
 ...
 <br>     
<br>
<b style="letter-spacing:1px; font-size:20px; font-family:'Calibri'">Title Selected : <l style="text-decoration:underline; color:#20B2AA;">Summary of Watershed Characterization, VA and IWMP, Region 10</l> </b>
<br>
                ...
                <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example" style="margin-top:-40px;">
                    ...
                    <br>
                        <tr>
                            <th style="width:60px; background-color:#FE4A49; color:white;">Options</th>
                            <th style="background-color:#FE4A49; color:white;">PENRO </th>
                            <th style="background-color:#FE4A49; color:white;">Total Number of River <br>( Basin/Watershed/Sub-Watershed )</th>
                            <th style="background-color:#FE4A49; color:white;">Status <br> (Profiling/Characterization)</th>
                            <th style="background-color:#FE4A49; color:white;">Status <br> (Vulnerability Assessment)</th>
                            <th style="background-color:#FE4A49; color:white;">Status <br> (IWMP Prepared)</th>
                    ...
                    <tbody>
                        ...
                        <?php
                        include('connect.php');
                        $result = $db->prepare("SELECT * FROM Summary_of_Watershed_Characterization");
                        $result->execute();
                        for($i=0; $row = $result->fetch(); $i++){
                            ?>

                        <tr onclick="window.location='Summary-of-Watershed-Characterization-VA-an-IWMP-edit?id=<?= $row['id'] ?>';" class="record">
                                <td><img src="img/Settings_96px.png" height="32" style="margin-left:20px;"></td>
                                <td><?php echo $row['PENRO']; ?></td>
                                <td><?php echo $row['Total_Number_of_River_Basin_Watershed_Sub_Watershed']; ?></td>
                                <td><?php echo $row['Status_Profiling_Characterization']; ?></td>
                                <td><?php echo $row['Status_Vulnerability_Assessment']; ?></td>
                                <td><?php echo $row['Status_IWMP_Prepared']; ?></td>
