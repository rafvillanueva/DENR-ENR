        ...
        <div class="sidebar-nav navbar-collapse">
            <ul class="nav" id="side-menu">
                ...
                <li style="border: 0px;"><br>
                    <a href="NIPAS-and-Locally-Managed-Marine-Protected-Areas">
...
if(isset($_GET['id'])){
  $id = $_GET['id'];
  $edit = mysqli_query($conn, "SELECT * FROM Nipas_and_locally_managed_marine_protected_areas WHERE id = '$id'");
  $row = mysqli_fetch_array($edit);
}elseif(isset($_GET['del'])){
    $id = $_GET['del'];
    $sql = mysqli_query($conn, "DELETE FROM Nipas_and_locally_managed_marine_protected_areas WHERE id = '$id'");
    header("location:NIPAS-and-Locally-Managed-Marine-Protected-Areas");
} 
?>
...
mysqli_query($conn,"UPDATE Nipas_and_locally_managed_marine_protected_areas set location_Province='$location_Province',No_of_MPAs='$No_of_MPAs',total_area_ha='$total_area_ha',year='$year',a_status='$a_status' where id='$id'");
echo "<script>alert('Success! Record Updated.'); window.location='NIPAS-and-Locally-Managed-Marine-Protected-Areas'</script>";
            ...
            <div class="panel-heading" style="letter-spacing:2px; font-size:20px; font-weight: bold;"><img src="img/Info Squared_96px.png"  height="40px" width="40px">EDIT RECORD</div>
                <div class="panel-body">
                       <form method="POST" class="form-horizontal" autocomplete="off">
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Location / Province</small></label>
                                       <div class="col-sm-10">          
                                          <input name="location_Province" type="text" class="form-control" required value="<?= $row['location_Province']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>No. of MPAs</small></label>
                                       <div class="col-sm-10">          
                                          <input name="No_of_MPAs" type="text" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" class="form-control" required value="<?= $row['No_of_MPAs']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Total Area (ha)</small></label>
                                       <div class="col-sm-10">          
                                          <input name="total_area_ha" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" type="text" class="form-control" required value="<?= $row['total_area_ha']; ?>">
                                       </div>
                              </div>


                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Field Status</small></label>
                                       <div class="col-sm-10">          
                                          <select name="a_status" class="form-control" required>
                                                          <option selected><?= $row['a_status'] ?></option>
                                                          <option value="" disabled>---</option>
                                                          <option value="NIPAS">NIPAS</option>
                                                          <option value="LGU-Managed MPAs">LGU-Managed MPAs </option>
                                          </select>
