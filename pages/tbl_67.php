     ...
     <div class="row1">
                      <label class="col-sm-12" style="font-weight:bold; margin-left:-15px; letter-spacing:1px; font-family:Calibri;font-size: 18px; color:#FE4A49;"> SELECT A RECORDS</label>
                            ...
                            <select class="form-control" onchange="myFunction()" id="mySelect" required style="border:1px solid gray; letter-spacing: 2px; font-size:17px;">
                              ...
                              <option selected value="1" class="title-op">Watershed Characterization/Profiling, Region 10</option>
...
<!--/ end -->
      ...
      <button class="btn btn-default btn btn-md" data-toggle="modal" data-target="#tbl67-add" style="height:40px; background:#001730; color:white; opacity:0.8;"><span class="fa fa-plus"></span> Add</button>


      <button class="btn btn-default btn btn-md"  data-toggle="modal" data-target="#tbl67-total"  style="background:#FE4A49; color:white; height:40px;"><span class="fa fa-calculator"></span> Total</button>



      <a href="report/tbl67.php">
 ...
 <br>     
<br>
<b style="letter-spacing:1px; font-size:20px; font-family:'Calibri'">Title Selected : <l style="text-decoration:underline; color:#20B2AA;">Watershed Forest Reserve (Proclaimed Watershed) in hectares, Region 10</l> </b>
<br>
                ...
                <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example" style="margin-top:-40px;">
                    ...
                    <br>
                        <tr>
                            ...
                            <th style="background-color:#FE4A49; color:white;"> <small>Name of Watershed</small></th>
                            <th style="background-color:#FE4A49; color:white;"> <small>City/Municiaplity</small></th>
                            <th style="background-color:#FE4A49; color:white;"> <small>Area (ha)</small></th>
                            <th style="background-color:#FE4A49; color:white;"> <small>Proclamation No.</small></th>
                            <th style="background-color:#FE4A49; color:white;"> <small>Proclamation Date (yyyy/mm/dd)</small></th>
                    ...
                    <tbody>
                        ...
                        <?php
                        include('connect.php');
                        $result = $db->prepare("SELECT * FROM Watershed_Forest_Reserve_Proclaimed_Watershed");
                        $result->execute();
                        for($i=0; $row = $result->fetch(); $i++){
                            ?>

                        <tr onclick="window.location='Watershed-Forest-Reserve-Proclaimed-Watershed-edit?id=<?= $row['id'] ?>';" class="record">
                                <td><img src="img/Settings_96px.png" height="32" style="margin-left:20px;"></td>
                                <td><?php echo $row['Province']; ?></td>
                                <td><?php echo $row['Name_of_Watershed']; ?></td>
                                <td><?php echo $row['City_Municiaplity']; ?></td>
                                <td><?php echo $row['Area_ha']; ?></td>
                                <td><?php echo $row['Proclamation_No']; ?></td>
                                <td><?php echo $row['Proclamation_Date']; ?></td>
