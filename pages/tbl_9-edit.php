...
if(isset($_GET['id'])){
  $id = $_GET['id'];
  $edit = mysqli_query($conn, "SELECT * FROM Cadastral_Survey WHERE id = '$id'");
  $row = mysqli_fetch_array($edit);
}elseif(isset($_GET['del'])){
    $id = $_GET['del'];
    $sql = mysqli_query($conn, "DELETE FROM Cadastral_Survey WHERE id = '$id'");
    header("location:Cadastral-Survey");
}
?>
...
mysqli_query($conn,"UPDATE Cadastral_Survey set Uncontested='$Uncontested',Contested='$Contested',Municipality='$Municipality',Province='$Province' where id='$id'");
echo "<script>alert('Success! Record Updated.'); window.location='Cadastral-Survey'</script>";
            ...
            <div class="panel-heading" style="letter-spacing:2px; font-size:20px; font-weight: bold;"><img src="img/Info Squared_96px.png"  height="40px" width="40px">EDIT RECORD</div>
                  <div class="panel-body">
                       <form method="POST" class="form-horizontal" autocomplete="off">
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Contested</small></label>
                                       <div class="col-sm-10">          
                                          <input name="Contested" type="text" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" class="form-control" required value="<?= $row['Contested']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Uncontested</small></label>
                                       <div class="col-sm-10">          
                                          <input name="Uncontested" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" type="text" class="form-control" required value="<?= $row['Uncontested']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
