        ...
        <div class="sidebar-nav navbar-collapse">
            <ul class="nav" id="side-menu">
                ...
                <li style="border: 0px;"><br>
                    <a href="Watershed-Forest-Reserve-Proclaimed-Watershed">
...
if(isset($_GET['id'])){
  $id = $_GET['id'];
  $edit = mysqli_query($conn, "SELECT * FROM Watershed_Forest_Reserve_Proclaimed_Watershed WHERE id = '$id'");
  $row = mysqli_fetch_array($edit);
}elseif(isset($_GET['del'])){
    $id = $_GET['del'];
    $sql = mysqli_query($conn, "DELETE FROM Watershed_Forest_Reserve_Proclaimed_Watershed WHERE id = '$id'");
    header("location:Watershed-Forest-Reserve-Proclaimed-Watershed");
} 
?>
...
mysqli_query($conn,"UPDATE Watershed_Forest_Reserve_Proclaimed_Watershed set Province='$Province',Name_of_Watershed='$Name_of_Watershed',City_Municiaplity='$City_Municiaplity',Area_ha='$Area_ha',Proclamation_No='$Proclamation_No',Proclamation_Date='$Proclamation_Date' where id='$id'");
echo "<script>alert('Success! Record Updated.'); window.location='Watershed-Forest-Reserve-Proclaimed-Watershed'</script>";
                       ...
                       <form method="POST" class="form-horizontal" autocomplete="off">
                              ...
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Name of Watershed</small></label>
                                       <div class="col-sm-10">          
                                          <input name="Name_of_Watershed" type="text" class="form-control" required value="<?= $row['Name_of_Watershed']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>City/Municiaplity</small></label>
                                       <div class="col-sm-10">          
                                          <input name="City_Municiaplity" type="text" class="form-control" required value="<?= $row['City_Municiaplity']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Area (ha)</small></label>
                                       <div class="col-sm-10">          
                                          <input name="Area_ha" type="text" class="form-control" required value="<?= $row['Area_ha']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Proclamation No.</small></label>
                                       <div class="col-sm-10">          
                                          <input name="Proclamation_No" type="text" class="form-control" required value="<?= $row['Proclamation_No']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Proclamation Date</small></label>
                                       <div class="col-sm-10">          
                                          <input name="Proclamation_Date" type="date" class="form-control" required value="<?= $row['Proclamation_Date']; ?>">
                                       </div>
                              </div>
