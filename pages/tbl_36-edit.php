        ...
        <div class="sidebar-nav navbar-collapse">
            <ul class="nav" id="side-menu">
                ...
                <li style="border: 0px;"><br>
                    <a href="Forest-Protection-Officers-Peo">
...
if(isset($_GET['id'])){
  $id = $_GET['id'];
  $edit = mysqli_query($conn, "SELECT * FROM Forest_protection_officers_peo WHERE id = '$id'");
  $row = mysqli_fetch_array($edit);
}elseif(isset($_GET['del'])){
    $id = $_GET['del'];
    $sql = mysqli_query($conn, "DELETE FROM Forest_protection_officers_peo WHERE id = '$id'");
    header("location:Forest-Protection-Officers-Peo");
}
?>
...
mysqli_query($conn,"UPDATE Forest_protection_officers_peo set Forest_Protection_Officers_PEO='$Forest_Protection_Officers_PEO',Number_of_FPos_Peos_hired='$Number_of_FPos_Peos_hired',Year='$Year' where id='$id'");
echo "<script>alert('Success! Record Updated.'); window.location='Forest-Protection-Officers-Peo'</script>";
            ...
            <div class="panel-heading" style="letter-spacing:2px; font-size:20px; font-weight: bold;"><img src="img/Info Squared_96px.png"  height="40px" width="40px">EDIT RECORD</div>
                  <div class="panel-body">
                       <form method="POST" class="form-horizontal" autocomplete="off">
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Forest Protection Officers and PEOs</small></label>
                                       <div class="col-sm-10">          
                                          <input name="Forest_Protection_Officers_PEO" type="text" class="form-control" required value="<?= $row['Forest_Protection_Officers_PEO']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Number of  FPOs/PEOs Hired</small></label>
                                       <div class="col-sm-10">          
                                          <input name="Number_of_FPos_Peos_hired" type="text" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" class="form-control" required value="<?= $row['Number_of_FPos_Peos_hired']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
