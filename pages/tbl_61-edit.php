        ...
        <div class="sidebar-nav navbar-collapse">
            <ul class="nav" id="side-menu">
                ...
                <li style="border: 0px;"><br>
                    <a href="Summary-of-CWR-Monitored">
...
if(isset($_GET['id'])){
  $id = $_GET['id'];
  $edit = mysqli_query($conn, "SELECT * FROM Summary_of_CWR_Monitored WHERE id = '$id'");
  $row = mysqli_fetch_array($edit);
}elseif(isset($_GET['del'])){
    $id = $_GET['del'];
    $sql = mysqli_query($conn, "DELETE FROM Summary_of_CWR_Monitored WHERE id = '$id'");
    header("location:Summary-of-CWR-Monitored");
} 
?>
...
mysqli_query($conn,"UPDATE Summary_of_CWR_Monitored set OperatingUnit='$OperatingUnit',No_of_CWR_WFP='$No_of_CWR_WFP',Year='$Year' where id='$id'");
echo "<script>alert('Success! Record Updated.'); window.location='Summary-of-CWR-Monitored'</script>";
            ...
            <div class="panel-heading" style="letter-spacing:2px; font-size:20px; font-weight: bold;"><img src="img/Info Squared_96px.png"  height="40px" width="40px">EDIT RECORD</div>
                  <div class="panel-body">
                       <form method="POST" class="form-horizontal" autocomplete="off">
                              ...
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>No. of CWR/WFP</small></label>
                                       <div class="col-sm-10">          
                                          <input name="No_of_CWR_WFP" type="text" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" class="form-control" required value="<?= $row['No_of_CWR_WFP']; ?>">
                                       </div>
                              </div>
