...
if(isset($_GET['id'])){
  $id = $_GET['id'];
  $edit = mysqli_query($conn, "SELECT * FROM apprehended_wood_charcoal WHERE id = '$id'");
  $row = mysqli_fetch_array($edit);
}elseif(isset($_GET['del'])){
    $id = $_GET['del'];
    $sql = mysqli_query($conn, "DELETE FROM apprehended_wood_charcoal WHERE id = '$id'");
    header("location:Apprehended-Wood-Charcoal");
}
?>
...
mysqli_query($conn,"UPDATE apprehended_wood_charcoal set Volume_inSacks='$Volume_inSacks',EstValue='$EstValue',Year='$Year',OperatingUnit='$OperatingUnit' where id='$id'");
echo "<script>alert('Success! Record Updated.'); window.location='Apprehended-Wood-Charcoal'</script>";
            ...
            <div class="panel-heading" style="letter-spacing:2px; font-size:20px; font-weight: bold;"><img src="img/Info Squared_96px.png"  height="40px" width="40px">EDIT RECORD</div>
                  <div class="panel-body">
                       <form method="POST" class="form-horizontal" autocomplete="off">
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Volume (in Sacks)</small></label>
                                       <div class="col-sm-10">          
                                          <input name="Volume_inSacks" type="text" class="form-control" required value="<?= $row['Volume_inSacks']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
