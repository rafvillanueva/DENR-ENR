        ...
        <div class="sidebar-nav navbar-collapse">
            <ul class="nav" id="side-menu">
                ...
                <li style="border: 0px;"><br>
                    <a href="Management-of-Foreshore">
...
if(isset($_GET['id'])){
  $id = $_GET['id'];
  $edit = mysqli_query($conn, "SELECT * FROM management_of_foreshore_areas_processed_and_endorsed WHERE id = '$id'");
  $row = mysqli_fetch_array($edit);
}elseif(isset($_GET['del'])){
    $id = $_GET['del'];
    $sql = mysqli_query($conn, "DELETE FROM management_of_foreshore_areas_processed_and_endorsed WHERE id = '$id'");
    header("location:Management-of-Foreshore");
}
?>
...
mysqli_query($conn,"UPDATE management_of_foreshore_areas_processed_and_endorsed set Operating_unit='$Operating_unit',no_of_foreshore_area='$no_of_foreshore_area',area_ha='$area_ha',year='$year' where id='$id'");
echo "<script>alert('Success! Record Updated.'); window.location='Management-of-Foreshore'</script>";
            ...
            <div class="panel-heading" style="letter-spacing:2px; font-size:20px; font-weight: bold;"><img src="img/Info Squared_96px.png"  height="40px" width="40px">EDIT RECORD</div>
                <div class="panel-body">
                       <form method="POST" class="form-horizontal" autocomplete="off">
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>PENRO/CENRO</small></label>
                                       <div class="col-sm-10">          
                                          <input name="Operating_unit" type="text" class="form-control" required value="<?= $row['Operating_unit']; ?>">
                                       </div>
                              </div>

                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>No. of Foreshore Area</small></label>
                                       <div class="col-sm-10">          
                                          <input name="no_of_foreshore_area" type="text" class="form-control" required value="<?= $row['no_of_foreshore_area']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
                              ...
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Year</small></label>
                                       <div class="col-sm-10">          
                                          <input name="year" type="text" class="form-control" required value="<?= $row['year']; ?>">
                                       </div>
                              </div>
