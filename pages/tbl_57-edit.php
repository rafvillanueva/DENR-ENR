...
if(isset($_GET['id'])){
  $id = $_GET['id'];
  $edit = mysqli_query($conn, "SELECT * FROM number_of_waterbodies WHERE id = '$id'");
  $row = mysqli_fetch_array($edit);
}elseif(isset($_GET['del'])){
    $id = $_GET['del'];
    $sql = mysqli_query($conn, "DELETE FROM number_of_waterbodies WHERE id = '$id'");
    header("location:Number-of-Waterbodies");
} 
?>
...
mysqli_query($conn,"UPDATE number_of_waterbodies set waterbodies_by_category='$waterbodies_by_category',no_of_waterbodies='$no_of_waterbodies',no_classified_waterbodies='$no_classified_waterbodies',no_of_unclassified_waterbodies='$no_of_unclassified_waterbodies' where id='$id'");
echo "<script>alert('Success! Record Updated.'); window.location='Number-of-Waterbodies'</script>";
            ...
            <div class="panel-heading" style="letter-spacing:2px; font-size:20px; font-weight: bold;"><img src="img/Info Squared_96px.png"  height="40px" width="40px">EDIT RECORD</div>
                <div class="panel-body">
                       <form method="POST" class="form-horizontal" autocomplete="off">
                              ...
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Waterbodies (By Category)</small></label>
                                       <div class="col-sm-10">          
                                          <input name="waterbodies_by_category" type="text" class="form-control" required value="<?= $row['waterbodies_by_category']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>No. of Waterbodies</small></label>
                                       <div class="col-sm-10">          
                                          <input name="no_of_waterbodies" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" type="text" class="form-control" required value="<?= $row['no_of_waterbodies']; ?>">
                                       </div>
                              </div>

                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>No. Classified Waterbodies</small></label>
                                       <div class="col-sm-10">          
                                          <input name="no_classified_waterbodies" type="text" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" class="form-control" required value="<?= $row['no_classified_waterbodies']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>No. of Unclassified Waterbodies</small></label>
                                       <div class="col-sm-10">          
                                          <input name="no_of_unclassified_waterbodies" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" type="text" class="form-control" required value="<?= $row['no_of_unclassified_waterbodies']; ?>">
                                       </div>
                              </div>                       
