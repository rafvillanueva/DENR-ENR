...
if(isset($_GET['id'])){
  $id = $_GET['id'];
  $edit = mysqli_query($conn, "SELECT * FROM existing_tfla WHERE id = '$id'");
  $row = mysqli_fetch_array($edit);
}elseif(isset($_GET['del'])){
    $id = $_GET['del'];
    $sql = mysqli_query($conn, "DELETE FROM existing_tfla WHERE id = '$id'");
    header("location:Existing-TFLA");
}
?>
...
mysqli_query($conn,"UPDATE existing_tfla set Penro='$Penro',Cenro='$Cenro',Name_of_holder='$Name_of_holder',Location='$Location',Area_ha='$Area_ha',Status='$Status' where id='$id'");
echo "<script>alert('Success! Record Updated.'); window.location='Existing-TFLA'</script>";
