...
if(isset($_GET['id'])){
  $id = $_GET['id'];
  $edit = mysqli_query($conn, "SELECT * FROM fireline_maintained WHERE id = '$id'");
  $row = mysqli_fetch_array($edit);
}elseif(isset($_GET['del'])){
    $id = $_GET['del'];
    $sql = mysqli_query($conn, "DELETE FROM fireline_maintained WHERE id = '$id'");
    header("location:Fireline-Maintained");
}
?>
...
mysqli_query($conn,"UPDATE fireline_maintained set Penro_Cenro='$Penro_Cenro',Area_ha='$Area_ha',Length_m='$Length_m',Year='$Year' where id='$id'");
echo "<script>alert('Success! Record Updated.'); window.location='Fireline-Maintained'</script>";
                       ...
                       <form method="POST" class="form-horizontal" autocomplete="off">
                              ...
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Operating Unit</small></label>
                                       <div class="col-sm-10">          
                                          <select name="Penro_Cenro" class="form-control" required>
                                                          <option selected><?= $row['Penro_Cenro'] ?></option>
                              ...
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Length (m)</small></label>
                                       <div class="col-sm-10">          
                                          <input name="Length_m" type="text" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" class="form-control" required value="<?= $row['Length_m']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
