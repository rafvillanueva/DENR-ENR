        ...
        <div class="sidebar-nav navbar-collapse">
            <ul class="nav" id="side-menu">
                ...
                <li style="border: 0px;"><br>
                    <a href="Classified-Principle-Rivers">
...
if(isset($_GET['id'])){
  $id = $_GET['id'];
  $edit = mysqli_query($conn, "SELECT * FROM classified_principle_rivers WHERE id = '$id'");
  $row = mysqli_fetch_array($edit);
}elseif(isset($_GET['del'])){
    $id = $_GET['del'];
    $sql = mysqli_query($conn, "DELETE FROM classified_principle_rivers WHERE id = '$id'");
    header("location:Classified-Principle-Rivers");
}
?>
...
mysqli_query($conn,"UPDATE classified_principle_rivers set Name_Rivers='$Name_Rivers',Category='$Category',Classification='$Classification',Location='$Location',Exit_point='$Exit_point' where id='$id'");
echo "<script>alert('Success! Record Updated.'); window.location='Classified-Principle-Rivers'</script>";
            ...
            <div class="panel-heading" style="letter-spacing:2px; font-size:20px; font-weight: bold;"><img src="img/Info Squared_96px.png"  height="40px" width="40px">EDIT RECORD</div>
                  <div class="panel-body">
                       <form method="POST" class="form-horizontal" autocomplete="off">
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Name of Rivers</small></label>
                                       <div class="col-sm-10">          
                                          <input name="Name_Rivers" type="text" class="form-control" required value="<?= $row['Name_Rivers']; ?>">
                                       </div>
                              </div>
                                      
                                          <input name="Category" type="hidden" class="form-control" required value="<?= $row['Category']; ?>">
