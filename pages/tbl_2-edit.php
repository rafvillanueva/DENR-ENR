        ...
        <div class="sidebar-nav navbar-collapse">
            <ul class="nav" id="side-menu">
                ...
                <li style="border: 0px;"><br>
                    <a href="Apprehended-Finished-Products">
...
if(isset($_GET['id'])){
  $id = $_GET['id'];
  $edit = mysqli_query($conn, "SELECT * FROM apprehended_finished_products WHERE id = '$id'");
  $row = mysqli_fetch_array($edit);
}elseif(isset($_GET['del'])){
    $id = $_GET['del'];
    $sql = mysqli_query($conn, "DELETE FROM apprehended_finished_products WHERE id = '$id'");
    header("location:Apprehended-Finished-Products");
}
?>
...
mysqli_query($conn,"UPDATE apprehended_finished_products set Volume_inPcs='$Volume_inPcs',EstValue='$EstValue',Year='$Year',OperatingUnit='$OperatingUnit' where id='$id'");
echo "<script>alert('Success! Record Updated.'); window.location='Apprehended-Finished-Products'</script>";
            ...
            <div class="panel-heading" style="letter-spacing:2px; font-size:20px; font-weight: bold;"><img src="img/Info Squared_96px.png"  height="40px" width="40px">EDIT RECORD</div>
                  <div class="panel-body">
                       <form method="POST" class="form-horizontal" autocomplete="off">
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Volume (in Pcs)</small></label>
                                       <div class="col-sm-10">          
                                          <input name="Volume_inPcs" type="text" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" class="form-control" required value="<?= $row['Volume_inPcs']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Est.Value (P)</small></label>
                                       <div class="col-sm-10">          
                                          <input name="EstValue" type="text" class="form-control" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" required value="<?= $row['EstValue']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Year</small></label>
                                       <div class="col-sm-10">          
                                          <input name="Year" type="text" class="form-control" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" required value="<?= $row['Year']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
