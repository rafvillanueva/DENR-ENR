        ...
        <div class="sidebar-nav navbar-collapse">
            <ul class="nav" id="side-menu">
                ...
                <li style="border: 0px;"><br>
                    <a href="INREMP-Natural-Resource-Management">
...
if(isset($_GET['id'])){
  $id = $_GET['id'];
  $edit = mysqli_query($conn, "SELECT * FROM Inremp_natural_resource_management_area_developed WHERE id = '$id'");
  $row = mysqli_fetch_array($edit);
}elseif(isset($_GET['del'])){
    $id = $_GET['del'];
    $sql = mysqli_query($conn, "DELETE FROM Inremp_natural_resource_management_area_developed WHERE id = '$id'");
    header("location:INREMP-Natural-Resource-Management");
}
?>
...
mysqli_query($conn,"UPDATE Inremp_natural_resource_management_area_developed set Province='$Province',Target='$Target',Accomplishment_ha='$Accomplishment_ha',Accomplishment_percent='$Accomplishment_percent',Year='$Year' where id='$id'");
echo "<script>alert('Success! Record Updated.'); window.location='INREMP-Natural-Resource-Management'</script>";
                       ...
                       <form method="POST" class="form-horizontal" autocomplete="off">
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Province</small></label>
                                       <div class="col-sm-10">          
                                          <input name="Province" type="text" class="form-control" required value="<?= $row['Province']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Target (ha)</small></label>
                                       <div class="col-sm-10">          
                                          <input name="Target" type="text" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" class="form-control" required value="<?= $row['Target']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Accomplishment (ha)</small></label>
                                       <div class="col-sm-10">          
                                          <input name="Accomplishment_ha" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" type="text" class="form-control" required value="<?= $row['Accomplishment_ha']; ?>">
                                       </div>
                              </div>

                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Accomplishment (%)</small></label>
                                       <div class="col-sm-10">          
                                          <input name="Accomplishment_percent" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" type="text" class="form-control" required value="<?= $row['Accomplishment_percent']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
