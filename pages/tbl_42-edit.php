        ...
        <div class="sidebar-nav navbar-collapse">
            <ul class="nav" id="side-menu">
                ...
                <li style="border: 0px;"><br>
                    <a href="Land-Disposition-Issued">
...
if(isset($_GET['id'])){
  $id = $_GET['id'];
  $edit = mysqli_query($conn, "SELECT * FROM land_disposition_issued WHERE id = '$id'");
  $row = mysqli_fetch_array($edit);
}elseif(isset($_GET['del'])){
    $id = $_GET['del'];
    $sql = mysqli_query($conn, "DELETE FROM land_disposition_issued WHERE id = '$id'");
    header("location:Land-Disposition-Issued");
}
?>
...
mysqli_query($conn,"UPDATE land_disposition_issued set Province='$Province',Total_Residential_Patents_Issued_Patents_issued_no='$Total_Residential_Patents_Issued_Patents_issued_no',Total_Residential_Patents_Issued_Lot_ha='$Total_Residential_Patents_Issued_Lot_ha',Total_Agricultural_Patents_Issued_Patents_issued_no='$Total_Agricultural_Patents_Issued_Patents_issued_no',Total_Agricultural_Patents_Issued_Lot_ha='$Total_Agricultural_Patents_Issued_Lot_ha' where id='$id'");
echo "<script>alert('Success! Record Updated.'); window.location='Land-Disposition-Issued'</script>";
                ...
                <div class="panel-body">
                       <form method="POST" class="form-horizontal" autocomplete="off">
                              ...
                              <div class="form-group container-fluid">
                                ...
                                <label><small>Total Residential Patents Issued Patents Issued (No.)</small></label>
                                <input name="Total_Residential_Patents_Issued_Patents_issued_no" type="text" class="form-control" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');"  required value="<?= $row['Total_Residential_Patents_Issued_Patents_issued_no']; ?>">
                              
                                <label><small>Total Residential Patents Issued Lot (ha)</small></label>
                                <input name="Total_Residential_Patents_Issued_Lot_ha" type="text" class="form-control" required oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');"  value="<?= $row['Total_Residential_Patents_Issued_Lot_ha']; ?>">
                             
                                <label><small>Total Agricultural Patents Issued Patents Issued (No.)</small></label>
                                <input name="Total_Agricultural_Patents_Issued_Patents_issued_no" type="text" class="form-control" required oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');"  value="<?= $row['Total_Agricultural_Patents_Issued_Patents_issued_no']; ?>">
                            
                                <label><small>Total Agricultural Patents Issued Lot (ha)</small></label>
                                <input name="Total_Agricultural_Patents_Issued_Lot_ha" type="text" class="form-control" required oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');"  value="<?= $row['Total_Agricultural_Patents_Issued_Lot_ha']; ?>">
