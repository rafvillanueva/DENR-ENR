        ...
        <div class="sidebar-nav navbar-collapse">
            <ul class="nav" id="side-menu">
                ...
                <li style="border: 0px;"><br>
                    <a href="Confiscated-Rattan-Poles">
...
if(isset($_GET['id'])){
  $id = $_GET['id'];
  $edit = mysqli_query($conn, "SELECT * FROM confiscated_rattan_poles WHERE id = '$id'");
  $row = mysqli_fetch_array($edit);
}elseif(isset($_GET['del'])){
    $id = $_GET['del'];
    $sql = mysqli_query($conn, "DELETE FROM confiscated_rattan_poles WHERE id = '$id'");
    header("location:Confiscated-Rattan-Poles");
}
?>
...
mysqli_query($conn,"UPDATE confiscated_rattan_poles set Volume_inLinealMeters='$Volume_inLinealMeters',EstValue='$EstValue',Year='$Year',OperatingUnit='$OperatingUnit' where id='$id'");
echo "<script>alert('Success! Record Updated.'); window.location='Confiscated-Rattan-Poles'</script>";
            ...
            <div class="panel-heading" style="letter-spacing:2px; font-size:20px; font-weight: bold;"><img src="img/Info Squared_96px.png"  height="40px" width="40px">EDIT RECORD</div>
                  <div class="panel-body">
                       <form method="POST" class="form-horizontal" autocomplete="off">
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Volume (in Lineal Meters)</small></label>
                                       <div class="col-sm-10">          
                                          <input name="Volume_inLinealMeters" type="text" class="form-control" required value="<?= $row['Volume_inLinealMeters']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
