        ...
        <div class="sidebar-nav navbar-collapse">
            <ul class="nav" id="side-menu">
                ...
                <li style="border: 0px;"><br>
                    <a href="Proclaimed-Protected-Areas-under-NIPAS">
...
if(isset($_GET['id'])){
  $id = $_GET['id'];
  $edit = mysqli_query($conn, "SELECT * FROM Proclaimed_Protected_Areas_under_NIPAS WHERE id = '$id'");
  $row = mysqli_fetch_array($edit);
}elseif(isset($_GET['del'])){
    $id = $_GET['del'];
    $sql = mysqli_query($conn, "DELETE FROM Proclaimed_Protected_Areas_under_NIPAS WHERE id = '$id'");
    header("location:Proclaimed-Protected-Areas-under-NIPAS");
} 
?>
...
mysqli_query($conn,"UPDATE Proclaimed_Protected_Areas_under_NIPAS set Province='$Province',Name_of_protected_area='$Name_of_protected_area',Location='$Location',Pa='$Pa',Bufferzone='$Bufferzone',Total_area_ha='$Total_area_ha',Proclamation_no_date='$Proclamation_no_date',Legislation_no_date='$Legislation_no_date' where id='$id'");
echo "<script>alert('Success! Record Updated.'); window.location='Proclaimed-Protected-Areas-under-NIPAS'</script>";
            ...
            <div class="panel-heading" style="letter-spacing:2px; font-size:20px; font-weight: bold;"><img src="img/Info Squared_96px.png"  height="40px" width="40px">EDIT RECORD</div>
                <div class="panel-body">
                       <form method="POST" class="form-horizontal" autocomplete="off">
                              ...
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>PA Area(ha)</small></label>
                                       <div class="col-sm-10">          
                                          <input name="Pa" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" type="text" class="form-control" required value="<?= $row['Pa']; ?>">
                                       </div>
                              </div>                       

                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Bufferzone Area(ha)</small></label>
                                       <div class="col-sm-10">          
                                          <input name="Bufferzone" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" type="text" class="form-control" required value="<?= $row['Bufferzone']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Total Area (ha)</small></label>
                                       <div class="col-sm-10">          
                                          <input name="Total_area_ha" readonly oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" type="text" class="form-control" required value="<?= $row['Total_area_ha']; ?>">
                                       </div>
                              </div>

                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Proclamation No. (Date)</small></label>
                                       <div class="col-sm-10">          
                                          <input name="Proclamation_no_date" type="text" class="form-control" required value="<?= $row['Proclamation_no_date']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Legislation No. (Date)</small></label>
                                       <div class="col-sm-10">          
                                          <input name="Legislation_no_date" type="text" class="form-control" required value="<?= $row['Legislation_no_date']; ?>">
                                       </div>
                              </div>    
