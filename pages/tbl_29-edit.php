...
if(isset($_GET['id'])){
  $id = $_GET['id'];
  $edit = mysqli_query($conn, "SELECT * FROM existing_protection_area_community_brma WHERE id = '$id'");
  $row = mysqli_fetch_array($edit);
}elseif(isset($_GET['del'])){
    $id = $_GET['del'];
    $sql = mysqli_query($conn, "DELETE FROM existing_protection_area_community_brma WHERE id = '$id'");
    header("location:Existing-PACBRMA");
}
?>
...
mysqli_query($conn,"UPDATE existing_protection_area_community_brma set Province='$Province',Name_of_Protected_Area='$Name_of_Protected_Area',Name_of_PACBRMA_holder='$Name_of_PACBRMA_holder',Location_of_CBP_area='$Location_of_CBP_area',Area_ha='$Area_ha' where id='$id'");
echo "<script>alert('Success! Record Updated.'); window.location='Existing-PACBRMA'</script>";
            ...
            <div class="panel-heading" style="letter-spacing:2px; font-size:20px; font-weight: bold;"><img src="img/Info Squared_96px.png"  height="40px" width="40px">EDIT RECORD</div>
                  <div class="panel-body">
                       <form method="POST" class="form-horizontal" autocomplete="off">
                              ...
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Name of PACBRMA holder</small></label>
                                       <div class="col-sm-10">          
                                          <input name="Name_of_PACBRMA_holder" type="text" class="form-control" required value="<?= $row['Name_of_PACBRMA_holder']; ?>">
                                       </div>
                              </div>

                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Location of CBP area</small></label>
                                       <div class="col-sm-10">          
                                          <input name="Location_of_CBP_area" type="text" class="form-control" required value="<?= $row['Location_of_CBP_area']; ?>">
                                       </div>
                              </div>
