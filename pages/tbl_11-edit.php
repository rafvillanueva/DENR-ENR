...
if(isset($_GET['id'])){
  $id = $_GET['id'];
  $edit = mysqli_query($conn, "SELECT * FROM csc_within_cbfma WHERE id = '$id'");
  $row = mysqli_fetch_array($edit);
}elseif(isset($_GET['del'])){
    $id = $_GET['del'];
    $sql = mysqli_query($conn, "DELETE FROM csc_within_cbfma WHERE id = '$id'");
    header("location:CSC-within-CBFMA");
}
?>
...
mysqli_query($conn,"UPDATE csc_within_cbfma set OperatingUnit='$OperatingUnit',For_Renewal_Renewed='$For_Renewal_Renewed',For_ReEvaluation='$For_ReEvaluation',For_Transfer='$For_Transfer',For_Cancellation='$For_Cancellation',Total='$Total',area_ha='$area_ha',year='$year' where id='$id'");
echo "<script>alert('Success! Record Updated.'); window.location='CSC-within-CBFMA'</script>";
                       ...
                       <form method="POST" class="form-horizontal" autocomplete="off">
                              ...
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>For Renewal Renewed</small></label>
                                       <div class="col-sm-10">          
                                          <input name="For_Renewal_Renewed" type="text" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" class="form-control" required value="<?= $row['For_Renewal_Renewed']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>For Re-Evaluation</small></label>
                                       <div class="col-sm-10">          
                                          <input name="For_ReEvaluation" type="text" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" class="form-control" required value="<?= $row['For_ReEvaluation']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>For Transfer</small></label>
                                       <div class="col-sm-10">          
                                          <input name="For_Transfer" type="text" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" class="form-control" required value="<?= $row['For_Transfer']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>For Cancellation</small></label>
                                       <div class="col-sm-10">          
                                          <input name="For_Cancellation" type="text" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" class="form-control" required value="<?= $row['For_Cancellation']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
