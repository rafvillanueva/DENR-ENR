        ...
        <div class="sidebar-nav navbar-collapse">
            <ul class="nav" id="side-menu">
                ...
                <li style="border: 0px;"><br>
                    <a href="Watershed-Characterization-Profiling">
...
if(isset($_GET['id'])){
  $id = $_GET['id'];
  $edit = mysqli_query($conn, "SELECT * FROM Watershed_Characterization_Profiling WHERE id = '$id'");
  $row = mysqli_fetch_array($edit);
}elseif(isset($_GET['del'])){
    $id = $_GET['del'];
    $sql = mysqli_query($conn, "DELETE FROM Watershed_Characterization_Profiling WHERE id = '$id'");
    header("location:Watershed-Characterization-Profiling");
} 
?>
...
mysqli_query($conn,"UPDATE Watershed_Characterization_Profiling set Year='$Year',Province='$Province',Name_of_Watershed='$Name_of_Watershed',City_Municipality_Barangay='$City_Municipality_Barangay',Area_ha='$Area_ha' where id='$id'");
echo "<script>alert('Success! Record Updated.'); window.location='Watershed-Characterization-Profiling'</script>";
            ...
            <div class="panel-heading" style="letter-spacing:2px; font-size:20px; font-weight: bold;"><img src="img/Info Squared_96px.png"  height="40px" width="40px">EDIT RECORD</div>
                  <div class="panel-body">
                       <form method="POST" class="form-horizontal" autocomplete="off">
                              ...
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>City/Municipalit /Barangay</small></label>
                                       <div class="col-sm-10">          
                                          <input name="City_Municipality_Barangay" type="text" class="form-control" required value="<?= $row['City_Municipality_Barangay']; ?>">
                                       </div>
                              </div>
