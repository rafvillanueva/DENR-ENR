...
if(isset($_GET['id'])){
  $id = $_GET['id'];
  $edit = mysqli_query($conn, "SELECT * FROM area_developed WHERE id = '$id'");
  $row = mysqli_fetch_array($edit);
}elseif(isset($_GET['del'])){
    $id = $_GET['del'];
    $sql = mysqli_query($conn, "DELETE FROM area_developed WHERE id = '$id'");
    header("location:Area-developed");
}
?>
...
mysqli_query($conn,"UPDATE area_developed set Province='$Province',Universe='$Universe',Baseline='$Baseline',NGP_Accomplishment='$NGP_Accomplishment',Inremp='$Inremp',CBFM_CARP='$CBFM_CARP',Tenurial_Instrument='$Tenurial_Instrument',Total='$Total' where id='$id'");
echo "<script>alert('Success! Record Updated.'); window.location='Area-developed'</script>";
                  ...
                  <div class="panel-body">
                       <form method="POST" class="form-horizontal" autocomplete="off">
                              <div class="form-group col-xs-12">
                                  <label class="control-label col-sm-2" for="pwd"><small>Universe</small></label>
                                       <div class="col-sm-10">          
                                          <input name="Universe" type="text" class="form-control" required value="<?= $row['Universe']; ?>">
                                       </div>
                              </div>
                              <div class="form-group col-xs-12">
                                  <label class="control-label col-sm-2" for="pwd"><small>Baseline</small></label>
                                       <div class="col-sm-10">          
                                          <input name="Baseline" type="text" class="form-control" required value="<?= $row['Baseline']; ?>">
                                       </div>
                              </div>

                              <div class="form-group col-xs-12">
                                  <label class="control-label col-sm-2" for="pwd"><small>NGP Accomplishment</small></label>
                                       <div class="col-sm-10">          
                                          <input name="NGP_Accomplishment" type="text" class="form-control" required value="<?= $row['NGP_Accomplishment']; ?>">
                                       </div>
                              </div>

                              <div class="form-group col-xs-12">
                                  <label class="control-label col-sm-2" for="pwd"><small>Inremp</small></label>
                                       <div class="col-sm-10">          
                                          <input name="Inremp" type="text" class="form-control" required value="<?= $row['Inremp']; ?>">
                                       </div>
                              </div>
 
                              <div class="form-group col-xs-12">
                                  <label class="control-label col-sm-2" for="pwd"><small>CBFM-CARP</small></label>
                                       <div class="col-sm-10">          
                                          <input name="CBFM_CARP" type="text" class="form-control" required value="<?= $row['CBFM_CARP']; ?>">
                                       </div>
                              </div>

                              <div class="form-group col-xs-12">
                                  <label class="control-label col-sm-2" for="pwd"><small>Tenurial Instrument</small></label>
                                       <div class="col-sm-10">          
                                          <input name="Tenurial_Instrument" type="text" class="form-control" required value="<?= $row['Tenurial_Instrument']; ?>">
                                       </div>
                              </div>
