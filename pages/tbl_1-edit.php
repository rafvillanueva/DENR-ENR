        ...
        <div class="sidebar-nav navbar-collapse">
            <ul class="nav" id="side-menu">
                ...
                <li style="border: 0px;"><br>
                    <a href="Apprehended-Conveyances">
...
if(isset($_GET['id'])){
  $id = $_GET['id'];
  $edit = mysqli_query($conn, "SELECT * FROM apprehended_conveyances WHERE id = '$id'");
  $row = mysqli_fetch_array($edit);
}elseif(isset($_GET['del'])){
    $id = $_GET['del'];
    $sql = mysqli_query($conn, "DELETE FROM apprehended_conveyances WHERE id = '$id'");
    header("location:Apprehended-Conveyances");
} 
?>
...
mysqli_query($conn,"UPDATE apprehended_conveyances set NoUnits='$NoUnits',EstValue='$EstValue',Year='$Year',OperatingUnit='$OperatingUnit' where id='$id'");
echo "<script>alert('Success! Record Updated.'); window.location='Apprehended-Conveyances'</script>";
                       ...
                       <form method="POST" class="form-horizontal" autocomplete="off">
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>No. of Units</small></label>
                                       <div class="col-sm-10">          
                                          <input name="NoUnits" type="text" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" class="form-control" required value="<?= $row['NoUnits']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Est.Value (P)</small></label>
                                       <div class="col-sm-10">          
                                          <input name="EstValue" type="text" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" class="form-control" required value="<?= $row['EstValue']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
