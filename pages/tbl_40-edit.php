...
if(isset($_GET['id'])){
  $id = $_GET['id'];
  $edit = mysqli_query($conn, "SELECT * FROM key_biodiversity_areas WHERE id = '$id'");
  $row = mysqli_fetch_array($edit);
}elseif(isset($_GET['del'])){
    $id = $_GET['del'];
    $sql = mysqli_query($conn, "DELETE FROM key_biodiversity_areas WHERE id = '$id'");
    header("location:Key-Biodiversity-Areas");
}
?>
...
mysqli_query($conn,"UPDATE key_biodiversity_areas set Province='$Province',Key_Biodiversity_areas='$Key_Biodiversity_areas',Area_ha='$Area_ha',Legal_basis='$Legal_basis' where id='$id'");
echo "<script>alert('Success! Record Updated.'); window.location='Key-Biodiversity-Areas'</script>";
            ...
            <div class="panel-heading" style="letter-spacing:2px; font-size:20px; font-weight: bold;"><img src="img/Info Squared_96px.png"  height="40px" width="40px">EDIT RECORD</div>
                  <div class="panel-body">
                       <form method="POST" class="form-horizontal" autocomplete="off">
                              ...
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Key Biodiversity Areas</small></label>
                                       <div class="col-sm-10">          
                                          <input name="Key_Biodiversity_areas" type="text" class="form-control" required value="<?= $row['Key_Biodiversity_areas']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
                              ...
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Legal Basis</small></label>
                                       <div class="col-sm-10">          
                                          <input name="Legal_basis" type="text" class="form-control" required value="<?= $row['Legal_basis']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">        
