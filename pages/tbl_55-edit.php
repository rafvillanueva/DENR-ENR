        ...
        <div class="sidebar-nav navbar-collapse">
            <ul class="nav" id="side-menu">
                ...
                <li style="border: 0px;"><br>
                    <a href="Number-of-Forest-Rangers">
...
if(isset($_GET['id'])){
  $id = $_GET['id'];
  $edit = mysqli_query($conn, "SELECT * FROM number_of_forest_ranger WHERE id = '$id'");
  $row = mysqli_fetch_array($edit);
}elseif(isset($_GET['del'])){
    $id = $_GET['del'];
    $sql = mysqli_query($conn, "DELETE FROM number_of_forest_ranger WHERE id = '$id'");
    header("location:Number-of-Forest-Rangers");
} 
?>
...
mysqli_query($conn,"UPDATE number_of_forest_ranger set OperatingUnit='$OperatingUnit',forest_rangers_no='$forest_rangers_no',teams_no='$teams_no',year='$year' where id='$id'");
echo "<script>alert('Success! Record Updated.'); window.location='Number-of-Forest-Rangers'</script>";
            ...
            <div class="panel-heading" style="letter-spacing:2px; font-size:20px; font-weight: bold;"><img src="img/Info Squared_96px.png"  height="40px" width="40px">EDIT RECORD</div>
                <div class="panel-body">
                       <form method="POST" class="form-horizontal" autocomplete="off">
                              ...
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Forest Rangers (No.)</small></label>
                                       <div class="col-sm-10">          
                                          <input name="forest_rangers_no" type="text" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" class="form-control" required value="<?= $row['forest_rangers_no']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Teams (No.)</small></label>
                                       <div class="col-sm-10">          
                                          <input name="teams_no" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" type="text" class="form-control" required value="<?= $row['teams_no']; ?>">
                                       </div>
                              </div>
