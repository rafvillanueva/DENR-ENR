        ...
        <div class="sidebar-nav navbar-collapse">
            <ul class="nav" id="side-menu">
                ...
                <li style="border: 0px;"><br>
                    <a href="Survival-rate-Percentage">
...
if(isset($_GET['id'])){
  $id = $_GET['id'];
  $edit = mysqli_query($conn, "SELECT * FROM ngp_survival_rate_percentage WHERE id = '$id'");
  $row = mysqli_fetch_array($edit);
}elseif(isset($_GET['del'])){
    $id = $_GET['del'];
    $sql = mysqli_query($conn, "DELETE FROM ngp_survival_rate_percentage WHERE id = '$id'");
    header("location:Survival-rate-Percentage");
}
?>
...
mysqli_query($conn,"UPDATE ngp_survival_rate_percentage set penro_cenro_pa='$penro_cenro_pa',percentage='$percentage',year='$year' where id='$id'");
echo "<script>alert('Success! Record Updated.'); window.location='Survival-rate-Percentage'</script>";
                ...
                <div class="panel-body">
                       <form method="POST" class="form-horizontal" autocomplete="off">
                              ...
                              <div class="form-group container-fluid">
                                    <label for="usr"><small>PENRO/CENRO/PA</small></label>
                                          <select name="penro_cenro_pa" class="form-control" required>
                                                          <option selected><?= $row['penro_cenro_pa'] ?></option>
                                    ...
                                    <label for="usr"><small>Percentage</small> (%)</label>
                                    <input name="percentage" type="text" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" class="form-control" required value="<?= $row['percentage']; ?>">
