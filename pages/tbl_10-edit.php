...
if(isset($_GET['id'])){
  $id = $_GET['id'];
  $edit = mysqli_query($conn, "SELECT * FROM CSC_outside_CBFMA WHERE id = '$id'");
  $row = mysqli_fetch_array($edit);
}elseif(isset($_GET['del'])){
    $id = $_GET['del'];
    $sql = mysqli_query($conn, "DELETE FROM CSC_outside_CBFMA WHERE id = '$id'");
    header("location:CSC-outside-CBFMA");
}
?>
...
mysqli_query($conn,"UPDATE CSC_outside_CBFMA set OperatingUnit='$OperatingUnit',For_Renewal_Renewed='$For_Renewal_Renewed',For_ReEvaluation='$For_ReEvaluation',For_Transfer='$For_Transfer',For_Cancellation='$For_Cancellation',Total='$Total',area_ha='$area_ha',year='$year' where id='$id'");
echo "<script>alert('Success! Record Updated.'); window.location='CSC-outside-CBFMA'</script>";
            ...
            <div class="panel-heading" style="letter-spacing:2px; font-size:20px; font-weight: bold;"><img src="img/Info Squared_96px.png"  height="40px" width="40px">EDIT RECORD</div>
                  <div class="panel-body">
                       <form method="POST" class="form-horizontal" autocomplete="off">
                              ...
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>For Renewal Renewed</small></label>
                                       <div class="col-sm-10">          
                                          <input name="For_Renewal_Renewed"  oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" type="text" class="form-control" required value="<?= $row['For_Renewal_Renewed']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>For Re-Evaluation</small></label>
                                       <div class="col-sm-10">          
                                          <input name="For_ReEvaluation" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" type="text" class="form-control" required value="<?= $row['For_ReEvaluation']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
                              <label class="control-label col-sm-2" for="pwd"><small>For Transfer</small></label>
                                       <div class="col-sm-10">          
                                          <input name="For_Transfer" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" type="text" class="form-control" required value="<?= $row['For_Transfer']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>For Cancellation</small></label>
                                       <div class="col-sm-10">          
                                          <input name="For_Cancellation" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" type="text" class="form-control" required value="<?= $row['For_Cancellation']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Area (ha)</small></label>
                                       <div class="col-sm-10">          
                                          <input name="area_ha" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" type="text" class="form-control" required value="<?= $row['area_ha']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">
                                  <label class="control-label col-sm-2" for="pwd"><small>Year</small></label>
                                       <div class="col-sm-10">          
                                          <input name="year" oninput="this.value = this.value.replace(/[^0-9.]/g, '').replace(/(\..*)\./g, '$1');" type="text" class="form-control" required value="<?= $row['year']; ?>">
                                       </div>
                              </div>
                              <div class="form-group">        
