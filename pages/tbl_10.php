     ...
     <div class="row1">
                      <label class="col-sm-12" style="font-weight:bold; margin-left:-15px; letter-spacing:1px; font-family:Calibri;font-size: 18px; color:#FE4A49;"> SELECT A RECORDS</label>
                            ...
                            <select class="form-control" onchange="myFunction()" id="mySelect" required style="border:1px solid gray; letter-spacing: 2px; font-size:17px;">
                              ...
                              <option selected value="1" class="title-op">Certificate of Stewardship Contract (CSC) Evaluated/Assessed outside CBFMA, Region 10</option>
...
<!--/ end -->
      ...
      <button class="btn btn-default btn btn-md" data-toggle="modal" data-target="#tbl10-add" style="height:40px; background:#001730; color:white; opacity:0.8;"><span class="fa fa-plus"></span> Add</button>

      <button class="btn btn-default btn btn-md"  data-toggle="modal" data-target="#tbl10-total"  style="background:#FE4A49; color:white; height:40px;"><span class="fa fa-calculator"></span> Total</button>


      <a href="report/tbl10.php">
 ...
 <br>     
<br>
<b style="letter-spacing:1px; font-size:20px; font-family:'Calibri';">Title Selected : <l style="text-decoration:underline; color:#20B2AA;">Certificate of Stewardship Contract (CSC) Evaluated/Assessed outside CBFMA, Region 10</l> </b>
<br>
                <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                    ...
                    <br>
                        <tr>
                            <th style="width:90px; background-color:#FE4A49; color:white;"> <small>Options</small> </th>
                            <th style="background-color:#FE4A49; color:white;"> <small>Operating Unit</small></th>
                            <th style="background-color:#FE4A49; color:white;"> <small>For Renewal Renewed</small></th>
                            <th style="background-color:#FE4A49; color:white;"> <small>For Re-Evaluation</small></th>
                            <th style="background-color:#FE4A49; color:white;"> <small>For Transfer</small></th>
                            <th style="background-color:#FE4A49; color:white;"> <small>For Cancellation</small></th>
                            <th style="background-color:#FE4A49; color:white;"> <small>Total</small></th>
                            <th style="background-color:#FE4A49; color:white;"> <small>Area (ha)</small></th>
                            <th style="background-color:#FE4A49; color:white;"> <small>Year</small></th>                        
                        </tr>
                    </thead>
                    <tbody>
                        ...
                        <?php
                        include('connect.php');
                        $result = $db->prepare("SELECT * FROM CSC_outside_CBFMA");
                        $result->execute();
                        for($i=0; $row = $result->fetch(); $i++){
                            ?>

                        <tr onclick="window.location='CSC-outside-CBFMA-edit?id=<?= $row['id'] ?>';" class="record">
                                <td><img src="img/Settings_96px.png" height="32" style="margin-left:20px;"></td>
                                <td><?php echo $row['OperatingUnit']; ?></td>
                                <td><?php echo number_format($row['For_Renewal_Renewed']); ?></td>
                                <td><?php echo number_format($row['For_ReEvaluation']); ?></td>
                                <td><?php echo number_format($row['For_Transfer']); ?></td>
                                <td><?php echo number_format($row['For_Cancellation']); ?></td>
                                <td><?php echo number_format($row['Total']); ?></td>
                                <td><?php echo number_format($row['area_ha']); ?></td>
